...
;; Design a function that determines if a sequence of characters
;; matches a given pattern.


;; Problem:
;;
;; It is often useful to be able to tell whether the first part of a
;; sequence of characters matches a given pattern. In this problem you
;; will design a (somewhat limited) way of doing this.
;;
...
(check-expect (pattern-match? empty empty) false)
(check-expect (pattern-match? empty (list "A")) false)
(check-expect (pattern-match? empty (list "N")) false)
(check-expect (pattern-match? (list "L") empty) false)
(check-expect (pattern-match? (list "1") empty) false)
(check-expect (pattern-match? (list "L") (list "A")) true)
(check-expect (pattern-match? (list "L") (list "N")) false)
(check-expect (pattern-match? (list "1") (list "N")) true)
(check-expect (pattern-match? (list "1") (list "A")) false)
(check-expect (pattern-match? (list "1") (list "N" "A")) false)
(check-expect (pattern-match? LOS1 PATTERN1) true)

;(define (pattern-match? lo1s p) false) ; Stub

(define (pattern-match? lo1s p)
  (cond [(empty? lo1s) false]
        [(empty? p) false]
        [(empty? (rest p)) (matches (first lo1s) (first p))]
        [else
         (and (matches (first lo1s) (first p))
              (pattern-match? (rest lo1s) (rest p)))]))

;; 1String 1String -> Boolean
;; produces true if 1String matches with pattern
(check-expect (matches " " " ") false)
(check-expect (matches "1" "A") false)
(check-expect (matches "A" "N") false)
(check-expect (matches "A" "A") true)
(check-expect (matches "1" "N") true)

;(define (matches s p) false) ; Stub

(define (matches s p)
  (cond [(string=? p "A") (alphabetic? s)]
        [(string=? p "N") (numeric? s)]
        [else false]))
