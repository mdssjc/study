...
(check-expect (lookup ACS1 0) #false)
(check-expect (lookup ACS2 0) #false)
...
(define (lookup accs n)
  (cond [(empty? accs) #false]
        [else
         (if (equals? (first accs) n)
             (account-name (first accs))
             (lookup (rest accs) n))]))

;; Account Natural -> Boolean
;; Check if account-num is equal to n
(check-expect (equals? (make-account 1 "abc") 0) #false)
(check-expect (equals? (make-account 1 "abc") 1) #true)

;(define (equals? acc n) #false) ; Stub
