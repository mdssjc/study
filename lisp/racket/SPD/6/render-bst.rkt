...
(check-expect (render-bst BST1) (above (text (string-append "1" KEY_VAL_SEPARATOR "abc")
                                             TEXT-SIZE
                                             TEXT-COLOR)
                                       VSPACE
                                       (beside (render-bst false)
                                               HSPACE
                                               (render-bst false))))
(check-expect (render-bst BST4) (above (text (string-append "4" KEY_VAL_SEPARATOR "dcj")
                                             TEXT-SIZE
                                             TEXT-COLOR)
                                       VSPACE
                                       (beside (render-bst false)
...
(check-expect (render-bst BST3) (above (text (string-append "3" KEY_VAL_SEPARATOR "ilk")
                                             TEXT-SIZE
                                             TEXT-COLOR)
                                       VSPACE
...
(define (render-bst t)
  (cond [(false? t) MTTREE]
        [else
         (above (text (string-append (number->string (node-key t)) KEY_VAL_SEPARATOR (node-val t))
                      TEXT-SIZE
                      TEXT-COLOR)
                VSPACE
