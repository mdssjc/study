...
;; above-all and beside-all functions defined below. Rewrite above-all
;; and beside-all using your abstract function.

;; (X Y -> Y) Y (listof X) -> Y
;; abstract function for arrange listof Image
(check-expect (arrange-all string-append "" (list "a" "bc" "def")) "abcdef")
(check-expect (arrange-all + 0 (list 1 2 3 4 5)) 15)
(check-expect (arrange-all cons empty (list "a" "b" "c"))
              (cons "a" (cons "b" (cons "c" empty))))
(check-expect (arrange-all above empty-image (list (square 10 "solid" "pink")
...
;; PROBLEM 2:
;;
;; Finish the design of the following functions, using built-in
;; abstract functions.
...
(define (minus-n lon n)
  (local ((define (sub-by-n x)
            (- x n)))
    (map sub-by-n lon)))

;; PROBLEM 3
;;
;; Consider the data definition below for Region. Design an abstract
;; fold function for region, and then use it do design a function that
...
;; please order the arguments in your fold function with combination
;; functions first, then bases, then region. Please number the bases
;; and combination functions in order of where they appear in the
;; function.
;;
...
(check-expect (fold-region make-region cons "Continent" "Country" "Province" "State" "City" empty CANADA)
              CANADA)
