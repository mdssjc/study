...
public final class StdRandom {
  ...
  public static int poisson(final double lambda) {
    ...
    var k = 0;
    var p = 1.0;
    final var expLambda = Math.exp(-lambda);
    do {
      k++;
      p *= uniform();
    } while (p >= expLambda);
   ...
   * @throws IllegalArgumentException
   *     if sum of frequencies exceeds {@code Integer.MAX_VALUE} (2<sup>31</sup>
   *     - 1)
   */
  ...
  public static void shuffle(final Object[] a) {
    ...
    for (var i = 0; i < n; i++) {
      final var r = i + uniform(n - i);     // between i and n-1
      final var temp = a[i];
      a[i] = a[r];
      a[r] = temp;
    }
  ...
  public static void shuffle(final Object[] a, final int lo, final int hi) {
    ...
    for (var i = lo; i < hi; i++) {
      final var r = i + uniform(hi - i);     // between i and hi-1
      final var temp = a[i];
      a[i] = a[r];
      a[r] = temp;
    }
  ...
  public static int[] permutation(final int n, final int k) {
    ...
    final var perm = new int[k];
    for (var i = 0; i < k; i++) {
      final var r = uniform(i + 1);    // between 0 and i
      perm[i] = perm[r];
      perm[r] = i;
    }
    for (var i = k; i < n; i++) {
  ...
  public static void main(final String[] args) {
    ...
    final var a = "A B C D E F G".split(" ");
