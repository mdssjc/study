...
public class BlogController {
  ...
  abstract class FreemarkerBasedRoute implements Route {
     ...
     * Constructor
     */
    protected FreemarkerBasedRoute(final String templateName)
        throws IOException {
  ...
  private void initializeRoutes() throws IOException {
    ...
    post("/signup", new FreemarkerBasedRoute("signup.ftl") {
      ...
      protected void doHandle(Request request, Response response, Writer writer)
        ...
        if (validateSignup(username, password, verify, email, root)) {
          // good user
          System.out.println(
              "Signup: Creating user with: " + username + " " + password);
          if (!userDAO.addUser(username, password, email)) {
            ...
            response.raw()
                    .addCookie(new Cookie("session", sessionID));
    ...
    post("/newpost", new FreemarkerBasedRoute("newpost_template.ftl") {
      ...
      protected void doHandle(Request request, Response response, Writer writer)
          throws IOException, TemplateException {
        String title = StringEscapeUtils.escapeHtml4(
        ...
        if (username == null) {
          response.redirect(
              "/login");    // only logged in users can post to blog
    ...
    get("/welcome", new FreemarkerBasedRoute("welcome.ftl") {
      ...
      protected void doHandle(Request request, Response response, Writer writer)
        ...
        if (username == null) {
          System.out.println(
              "welcome() can't identify the user, redirecting to signup");
    ...
    post("/newcomment", new FreemarkerBasedRoute("entry_template.ftl") {
      ...
      protected void doHandle(Request request, Response response, Writer writer)
        ...
        String name = StringEscapeUtils.escapeHtml4(
            request.queryParams("commentName"));
        String email = StringEscapeUtils.escapeHtml4(
            request.queryParams("commentEmail"));
        String body = StringEscapeUtils.escapeHtml4(
    ...
    post("/login", new FreemarkerBasedRoute("login.ftl") {
      ...
      protected void doHandle(Request request, Response response, Writer writer)
        ...
        System.out.println(
            "Login: User submitted: " + username + "  " + password);
  ...
  public boolean validateSignup(String username, String password, String verify,
                                String email, Map<String, String> errors) {
